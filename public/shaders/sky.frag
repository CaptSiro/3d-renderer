...
vec3 sun() {
    ...
    return s + sunDirection * atmosphereR;
}
...
vec3 light(vec3 start, vec3 direction, float len, vec3 color) {
    vec3 point = start;
    float stepSize = len / (float(LightPoints) - 1.0);
    vec3 scatteredLight = vec3(0.0);
    float viewRayOpticalDepth = 0.0;

    for (int i = 0; i < LightPoints; i++) {
        vec3 sunDirection = normalize(sun() - point);
        float sunRayLength = raySphereIntersection(center, atmosphereR, point, sunDirection).y;
...
void main() {
    ...
    atmosphereR = AtmosphereHeight + planetR;
    ...
    vec2 t = raySphereIntersection(center, atmosphereR, rayStart, rayDirection);
    ...
    if (throughAtmosphere > 0.0) {
        ...
        vec3 l = light(hit, rayDirection, throughAtmosphere - epsilon, vec3(0.0));
        OutColor = vec4(l, 1.0);
    } else {
